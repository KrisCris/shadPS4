...
static const char* GetMockResponseForPath(const std::string& path) {
    if (path.find("blockList") != std::string::npos) {
        return "{\"totalResults\": 0, \"blockList\": []}";
    }
    if (path.find("friendList") != std::string::npos) {
...
s32 sendRequest(s64 requestId, s32 partIndex, const void* pData, u64 dataSize, s8 flag,
    ...
    if (const char* mock_response = GetMockResponseForPath(request->userPath); mock_response != nullptr) {
        const size_t mock_size = std::min(sizeof(request->data), std::strlen(mock_response));
        std::memcpy(request->data, mock_response, mock_size);
        request->remainingData = mock_size;
...
s32 PS4_SYSV_ABI getHttpStatusCodeInternal(s64 requestId, s32* out_status_code) {
    ...
    if (request->sent && GetMockResponseForPath(request->userPath) != nullptr) {
