...
s32 GetAuthorizationCode(s32 req_id, const OrbisNpAuthGetAuthorizationCodeParameterA* param,
    ...
    constexpr auto kDummyAuthCode = "DUMMY-CODE";
    std::memset(auth_code, 0, sizeof(OrbisNpAuthorizationCode));
    std::memcpy(auth_code, kDummyAuthCode,
                std::min(sizeof(OrbisNpAuthorizationCode), sizeof(kDummyAuthCode)));
    if (issuer_id != nullptr) {
        *issuer_id = static_cast<s32>(std::strlen(kDummyAuthCode));
    }
    return ORBIS_OK;
}
